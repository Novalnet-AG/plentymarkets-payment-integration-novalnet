            ...
            if(in_array($transactionDetails['tx_status'], ['PENDING', 'CONFIRMED'])) {
                ...
                if($paymentResponseData['result']['status'] == 'SUCCESS' && in_array($paymentResponseData['transaction']['status'], ['PENDING', 'CONFIRMED', 'DEACTIVATED'])) {
                    ...
                    if(!empty($paymentResponseData['transaction']['refund']['tid'])) {
                        $paymentResponseData['bookingText'] = sprintf($this->paymentHelper->getTranslatedText('refund_message_new_tid', $orderLanguage), $paymentResponseData['transaction']['tid'], str_replace('.', ',', sprintf('%0.2f', ($paymentResponseData['transaction']['refund']['amount'] / 100))) , $paymentCurrency, $paymentResponseData['transaction']['refund']['tid']);
                    } else {
                        $paymentResponseData['bookingText'] = sprintf($this->paymentHelper->getTranslatedText('refund_message', $orderLanguage), $paymentResponseData['transaction']['tid'], str_replace('.', ',', sprintf('%0.2f', ($paymentResponseData['transaction']['refund']['amount'] / 100))), $paymentCurrency, uniqid());
                    }
