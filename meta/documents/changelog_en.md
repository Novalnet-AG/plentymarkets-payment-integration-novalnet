...
- Implemented Account Holder field for Direct Debit SEPA, Direct Debit SEPA with Payment Guarantee, and Instalment by Direct Debit SEPA, in compliance with Verification of Payee (VoP) regulations
- EPC/SPC QR code implemented for Prepayment, Invoice, Invoice with payment guarantee and Instalment by Invoice payments

### Fix

- Updated post data URL handling for domains with or without a trailing slash 

### Enhanced
- iDEAL payment logo and title to iDEAL | Wero
- Callback/Webhook logic updated for efficient handling of Dynamic IPs
- Optimized phone number and company data handling in payment request
- Implemented allowed countries and order amount limits configuration for all payment methods
- User interface for the event creation workflow 

### Remove 

- Sofort, giropay, Barzahlen/viacash and payconiq payment
...
- Google pay button is displayed properly when Other payment provider are active
- Novalnet transaction details are updated when creating an invoice using Document Builder

### Enhanced

- Extension of Instalment Payment cycles till 36 cycles
- Offering Instalment payments and Guarantee payments to B2B customers in Switzerland
...
- Minification of  JavaScript files for enhanced performance
