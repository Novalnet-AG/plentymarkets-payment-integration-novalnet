...
jQuery(document).ready( function() {
	...
	jQuery("#nn_instalment_date").on("keypress textInput",function (e) {
		...
		var day_val = jQuery('#nn_instalment_date').val();
		if(day_val.length == 1) {
	...
	jQuery('#nn_instalment_date').on('blur', function() {
		var date, updated_date;
		updated_date = date = jQuery('#nn_instalment_date').val();
		if(date != '0' && date != '' && date.length < 2) {
			updated_date = "0"+ date;
		} else if(date == '0') {
			updated_date = date.replace('0', '01');
		}
		jQuery('#nn_instalment_date').val(updated_date);
	});
	// Validate the customer birthday year and autoload the year
	jQuery("#nn_instalment_year").on("input", function(e) {
	...
	function autocompleteYear(input_val, array_year) {
		var currentFocus;
		...
		function addActiveValue(x) {
			...
			jQuery('#nn_instalment_year').val( jQuery('.year_active', elements).val() );
		}
	...
	autocompleteYear(document.getElementById("nn_instalment_year"), year_range);
	jQuery('#novalnet_form').on('submit', function() {
		jQuery('#novalnet_form_btn').prop('disabled', true);
		if(jQuery("#nn_instalment_year").val() == '' || jQuery("#nn_instalment_date").val() == '' ) {
			alert(jQuery("#nn_dob_empty").val());
			jQuery('#novalnet_form_btn').prop('disabled', false);
			return false;
		}
		if(jQuery("#nn_instalment_month").val() == '0') {
			alert(jQuery("#nn_dob_invalid").val());
			jQuery('#novalnet_form_btn').prop('disabled', false);
			return false;
		}
		return isActualDate(jQuery("#nn_instalment_month").val(), jQuery("#nn_instalment_date").val(), jQuery("#nn_instalment_year").val());
	});

	function isActualDate (month, day, year) {
		var tempDate = new Date(year, --month, day);
		if(month !== tempDate.getMonth() || jQuery("#nn_instalment_year").val().length < 4) {
			alert(jQuery("#nn_dob_invalid").val());
			jQuery('#novalnet_form_btn').prop('disabled', false);
			return false;
		}
